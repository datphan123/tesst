...
cc.Class({
	...
	onAccept: function(){
		if (this.SelectOne == false && this.SelectTwo == false && this.SelectThree == false) {
			cc.RedT.inGame.notice.show({title:"BK88 BET", text:'Chọn ít nhất một cửa để đặt cược!'});
		}else if(helper.isEmpty(this.editBoxTienCuoc.string)) {
			cc.RedT.inGame.notice.show({title:"BK88 BET", text:'Vui lòng nhập tiền cược!'});
		}
		else if(helper.getOnlyNumberInString(this.editBoxTienCuoc.string) < 1000){
			cc.RedT.inGame.notice.show({title:"BK88 BET", text:'Tiền cược phải ít nhất 1.000 XU!'});
		}
